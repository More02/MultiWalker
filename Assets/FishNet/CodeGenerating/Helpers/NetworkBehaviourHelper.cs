...
namespace FishNet.CodeGenerating.Helping
{
    internal class NetworkBehaviourHelper : CodegenBase
    {
        ...
        public MethodReference ObserversRpcDelegate_Ctor_MethodRef;
        //Is checks.
        public MethodReference IsClientInitialized_MethodRef;
        public MethodReference IsOwner_MethodRef;
        public MethodReference IsServerInitializedInitialized_MethodRef;
        ...
        public override bool ImportReferences()
        {
            ...
            //ServerRpcDelegate and ObserversRpcDelegate constructors.
            ServerRpcDelegate_Ctor_MethodRef = base.ImportReference(typeof(ServerRpcDelegate).GetConstructors().First());
            ObserversRpcDelegate_Ctor_MethodRef = base.ImportReference(typeof(ObserversRpcDelegate).GetConstructors().First());
            ...
            foreach (PropertyInfo pi in networkBehaviourType.GetProperties((BindingFlags.Static | BindingFlags.Public | BindingFlags.Instance | BindingFlags.NonPublic)))
            {
                ...
                else if (pi.Name == nameof(NetworkBehaviour.IsServerInitializedInitialized))
                    IsServerInitializedInitialized_MethodRef = base.ImportReference(pi.GetMethod);
        ...
        internal void CreateRpcDelegate(bool runLocally, TypeDefinition typeDef, MethodDefinition readerMethodDef, RpcType rpcType, uint methodHash, CustomAttribute rpcAttribute)
        {
            ...
            {
                insts.Add(processor.Create(OpCodes.Newobj, ObserversRpcDelegate_Ctor_MethodRef));
        ...
        /// </summary>
        /// <param name="useStatic">When true InstanceFinder.IsServerInitialized is used, when false base.IsServerInitializedInitialized is used.</param>
        internal void CreateIsServerInitializedCheck(MethodDefinition methodDef, LoggingType loggingType, bool useStatic, bool insertFirst, bool checkIsNetworked)
        {
            /* This is placed after the if check.
            * Should the if check pass then code
            * jumps to this instruction. */
            ...
            if (!useStatic)
            {
                ...
                instructions.Add(processor.Create(OpCodes.Call, IsServerInitializedInitialized_MethodRef));
            }
            //Checking instanceFinder.
            else
            {
                instructions.Add(processor.Create(OpCodes.Call, base.GetClass<ObjectHelper>().InstanceFinder_IsServerInitialized_MethodRef));
            }
            instructions.Add(processor.Create(OpCodes.Brtrue, endIf));
            //If warning then also append warning text.
            if (loggingType != LoggingType.Off)
