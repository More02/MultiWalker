    ...
    internal class RpcProcessor : CodegenBase
    {
        ...
        private MethodDefinition CreateRpcWriterMethod(List<ParameterDefinition> serializedParameters, List<AttributeData> datas, CreatedRpc cr, out bool intentionallyNull)
        {
            ...
            if (cr.RpcType == RpcType.Server)
                ...
                result = CreateObserversRpcWriterMethod(serializedParameters, datas, cr);
            else
                result = false;
        ...
        /// </summary>
        private bool CreateObserversRpcWriterMethod(List<ParameterDefinition> serializedParameters, List<AttributeData> attributeDatas, CreatedRpc cr)
        {
            ...
            /* Creates basic ServerRpc and ObserversRpc
             * conditions such as if requireOwnership ect..
             * or if (!base.isClient) */
            
                CreateObserversRpcConditionsForServer(writerMd);
        ...
        private MethodDefinition CreateRpcReaderMethod(List<ParameterDefinition> serializedParameters, List<AttributeData> datas, CreatedRpc cr, out bool intentionallyNull)
        {
            ...
            if (rpcType == RpcType.Server)
                ...
                return CreateObserversRpcReaderMethod(serializedParameters, datas, cr);
            else
                return null;
        }
        ...
        private MethodDefinition CreateServerRpcReaderMethod(TypeDefinition typeDef, bool runLocally, MethodDefinition originalMd, MethodDefinition createdMd, List<ParameterDefinition> serializedParameters, MethodDefinition logicMd, CustomAttribute rpcAttribute)
        {
            ...
            base.GetClass<NetworkBehaviourHelper>().CreateIsServerInitializedCheck(createdMd, LoggingType.Off, false, false, false);
            //
        ...
        /// <param name="rpcAttribute"></param>
        /// <returns></returns>
        private MethodDefinition CreateObserversRpcReaderMethod(List<ParameterDefinition> serializedParameters, List<AttributeData> attributeDatas, CreatedRpc cr)
        {
            MethodDefinition originalMd = cr.OriginalMethodDef;
        ...
        /// <summary>
        /// Creates conditions that server must pass to process a ObserversRpc.
        /// </summary>
        /// <param name="createdProcessor"></param>
        private void CreateObserversRpcConditionsForServer(MethodDefinition methodDef)
        {
            //If (!base.IsServerInitialized)
            base.GetClass<NetworkBehaviourHelper>().CreateIsServerInitializedCheck(methodDef, LoggingType.Warning, false, false, false);
        }
