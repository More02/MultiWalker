...
namespace FishNet.Object
{
    ...
    public abstract partial class NetworkBehaviour : MonoBehaviour
    {
        ...
        /// </summary>
        private readonly Dictionary<uint, ObserversRpcDelegate> _observersRpcDelegates = new Dictionary<uint, ObserversRpcDelegate>();
        /// <summary>
        /// Registered TargetRpc methods.
        /// </summary>
        private readonly Dictionary<uint, ObserversRpcDelegate> _targetRpcDelegates = new Dictionary<uint, ObserversRpcDelegate>();
        /// <summary>
        ...
        [CodegenMakePublic]
        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        internal void RegisterObserversRpc(uint hash, ObserversRpcDelegate del)
        {
            if (_observersRpcDelegates.TryGetValueIL2CPP(hash, out ObserversRpcDelegate currentDelegate))
            {
        ...
        [CodegenMakePublic]
        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        internal void RegisterTargetRpc(uint hash, ObserversRpcDelegate del)
        {
            if (_targetRpcDelegates.TryGetValueIL2CPP(hash, out ObserversRpcDelegate currentDelegate))
            {
        ...
        internal void OnObserversRpc(uint? methodHash, PooledReader reader, Channel channel)
        {
            ...
            if (_observersRpcDelegates.TryGetValueIL2CPP(methodHash.Value, out ObserversRpcDelegate del))
        ...
        internal void OnTargetRpc(uint? methodHash, PooledReader reader, Channel channel)
        {
            ...
            if (_targetRpcDelegates.TryGetValueIL2CPP(methodHash.Value, out ObserversRpcDelegate del))
                del.Invoke(reader, channel);
            else
