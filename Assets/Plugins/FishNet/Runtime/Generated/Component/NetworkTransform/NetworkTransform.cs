...
#pragma warning restore CS0414 //Not in use.
        ...
        {
            _sendToOwner = value;
            if (base.IsServerInitializedInitialized)
        ...
        {
            bool asClientAndNotHost = (!asServer && !base.IsServerInitialized);
        ...
        private void ConfigureComponents()
        {
            ...
            else if (_componentConfiguration == ComponentConfigurationType.CharacterController)
            {
                if (TryGetComponent<CharacterController>(out CharacterController c))
                {
                    ...
                    else
                    {
                        //Not CSP.
                        if (_sendToOwner)
                            c.enabled = base.IsServerInitializedInitialized;
                        //Most likely CSP.
                        else
                            c.enabled = (base.IsServerInitializedInitialized || base.IsOwner);
                    }
                }
            }
            ...
            {
                if (_clientAuthoritative)
                    return (!base.IsOwner || base.IsServerInitializedOnly);
                else
                    return !base.IsServerInitializedInitialized;
            }
        }
        ...
        {
            bool IsServerInitialized = base.IsServerInitializedInitialized;
        ...
        {
            bool canSet = (base.IsServerInitializedInitialized && !_clientAuthoritative)
                || (base.IsServerInitializedInitialized && _clientAuthoritative && !base.Owner.IsValid)
        ...
        private void MoveToTarget(float deltaOverride = -1f)
        {
            ...
            if (!base.IsServerInitializedInitialized && !base.IsClientInitialized)
                return;
            ...
            if (!controlledByClient && base.IsServerInitializedInitialized)
                return;
        ...
        private void TargetUpdateTransform(NetworkConnection conn, ArraySegment<byte> data, Channel channel)
        {
            ...
            if (base.IsServerInitializedInitialized && conn.IsLocalClient)
                return;
#endif
        ...
        public void SetSynchronizedProperties(SynchronizedProperty value)
        {
             ...
             */
            bool canSend = (
                base.IsServerInitializedInitialized ||
                (_clientAuthoritative && base.IsOwner)
                );
